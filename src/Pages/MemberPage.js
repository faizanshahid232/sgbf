...
import { deleteOrganizationMember, getAllOrganizationMember } from '../Redux/Slice/organizationMemberSlice';
...
export default function MemberPage() {
  ...
  const users = useSelector((s) => s.organizationMember?.data?.data);
  ...
  const getAllUser = async () => {
    const res = await dispatch(getAllOrganizationMember(id));
    if (res) {
      setRefetch(true);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    const res = await dispatch(deleteOrganizationMember(data));
    if (res.payload) {
      setOpenDelete(false);
      setDeleteId(null);
      setRefetch(!refetch);
  ...
  return (
    <>
      <Helmet>
        <title> Members | Saudi Green Building Forum </title>
      </Helmet>
      ...
      <Container maxWidth="xl">
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={1}>
          ...
          <Button
            ...
            onClick={() => navigate(`/dashboard/members/new-members/${id}`)}
          >
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)?.map((row) => {
                    const { _id, name, email, firstName, lastName } = row;
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>
                          <MenuItem
                            onClick={() => navigate(`/dashboard/members/edit-members/${id}`, { state: { item: row } })}
                          >
